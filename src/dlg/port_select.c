                                   ...
                                   LPARAM lParam)
{
    ...
    case WM_COMMAND:
        ...
        case IDOK: {
            ...
            if (sel < 0) {
                MessageBoxW(hDlg, LoadStr(IDS_MSG_SELECT_PORT),
            ...
            if (!Serial_GetPortName(portIdx, g_szSelectedPort, 32)) {
                MessageBoxW(hDlg, LoadStr(IDS_MSG_INVALID_PORT),
...
{
    INT_PTR ret =
        DialogBoxW(GetModuleHandle(NULL), MAKEINTRESOURCEW(IDD_PORT_SELECT),
                   hWnd, PortSelectDlgProc);
    if (ret == IDOK) {
        return TRUE;
    }
    return FALSE;
