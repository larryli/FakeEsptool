...
{
    switch (purpose) {
    case KEY_PURPOSE_USER:                return L"USER (0)";
    case KEY_PURPOSE_RESERVED:            return L"RESERVED (1)";
    case KEY_PURPOSE_XTS_AES_256_KEY_1:  return L"XTS-AES-256-1 (2)";
    case KEY_PURPOSE_XTS_AES_256_KEY_2:  return L"XTS-AES-256-2 (3)";
    case KEY_PURPOSE_XTS_AES_128_KEY:    return L"XTS-AES-128 (4)";
    case KEY_PURPOSE_HMAC_DOWN_ALL:      return L"HMAC-DOWN-ALL (5)";
    case KEY_PURPOSE_HMAC_DOWN_JTAG:     return L"HMAC-DOWN-JTAG (6)";
    case KEY_PURPOSE_HMAC_DOWN_DIGITAL_SIGNATURE: return L"HMAC-DOWN-SIG (7)";
    case KEY_PURPOSE_HMAC_UP:            return L"HMAC-UP (8)";
    case KEY_PURPOSE_SECURE_BOOT_DIGEST0: return L"SEC-BOOT-DIG0 (9)";
    case KEY_PURPOSE_SECURE_BOOT_DIGEST1: return L"SEC-BOOT-DIG1 (10)";
    case KEY_PURPOSE_SECURE_BOOT_DIGEST2: return L"SEC-BOOT-DIG2 (11)";
 ...
 */
static void FormatKeyHex(const CHIP_CTX *chip, int offset, int size, WCHAR *buf, int bufChars)
{
...
static void RefreshListView(HWND hList, int selectIndex)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        MultiByteToWideChar(CP_UTF8, 0, blocks[i].name, -1, wName, KEY_BLOCK_NAME_MAX);
        item.pszText = wName;
    ...
    if (hImport) EnableWindow(hImport, !connected && count > 0);
    if (hExport) EnableWindow(hExport, !connected && count > 0);
    if (hGenerate) EnableWindow(hGenerate, !connected && count > 0);
    if (hClear) EnableWindow(hClear, !connected && count > 0);
...
static void HandleImport(HWND hDlg, HWND hList)
{
    ...
    if (fileSize != (DWORD)blocks[sel].key_size) {
        WCHAR msg[128];
        wsprintfW(msg, LoadStr(IDS_KEY_MGMT_SIZE_MISMATCH),
                  fileSize, blocks[sel].key_size);
        MessageBoxW(hDlg, msg, LoadStr(IDS_KEY_MGMT_CAPTION), MB_OK | MB_ICONERROR);
        CloseHandle(hFile);
        return;
    }
    ...
    DWORD bytesRead;
    BOOL ok = ReadFile(hFile, key, (DWORD)blocks[sel].key_size, &bytesRead, NULL)
...
static void HandleExport(HWND hDlg, HWND hList)
{
    ...
    if (IsKeyEmpty(&g_device.chip, blocks[sel].efuse_offset, blocks[sel].key_size)) {
    ...
    /* Read key from eFuse */
    BYTE key[KEY_SIZE_MAX] = {0};
    ReadKey(&g_device.chip, blocks[sel].efuse_offset, blocks[sel].key_size, key);
    ...
    DWORD bytesWritten;
    BOOL ok = WriteFile(hFile, key, (DWORD)blocks[sel].key_size, &bytesWritten, NULL)
...
static void HandleGenerate(HWND hDlg, HWND hList)
{
    ...
    if (!IsKeyEmpty(&g_device.chip, blocks[sel].efuse_offset, blocks[sel].key_size)) {
        int ret = MessageBoxW(hDlg, LoadStr(IDS_KEY_MGMT_CONFIRM_OVERWRITE),
                              LoadStr(IDS_KEY_MGMT_CAPTION), MB_YESNO | MB_ICONQUESTION);
        if (ret != IDYES)
            return;
    }
...
static void HandleClear(HWND hDlg, HWND hList)
{
    ...
    /* Confirm clear */
    int ret = MessageBoxW(hDlg, LoadStr(IDS_KEY_MGMT_CONFIRM_CLEAR),
...
static void HandlePurpose(HWND hDlg, HWND hList)
{
    ...
    const BYTE purposes[] = {
        KEY_PURPOSE_USER, KEY_PURPOSE_XTS_AES_128_KEY,
        KEY_PURPOSE_XTS_AES_256_KEY_1, KEY_PURPOSE_XTS_AES_256_KEY_2,
        KEY_PURPOSE_HMAC_DOWN_ALL, KEY_PURPOSE_HMAC_DOWN_JTAG,
        KEY_PURPOSE_HMAC_DOWN_DIGITAL_SIGNATURE, KEY_PURPOSE_HMAC_UP,
        KEY_PURPOSE_SECURE_BOOT_DIGEST0, KEY_PURPOSE_SECURE_BOOT_DIGEST1,
    ...
    int chosen = TrackPopupMenu(hMenu, TPM_RETURNCMD | TPM_NONOTIFY,
...
static void HandleInitDialog(HWND hDlg)
{
    ...
    col.cx = 70;  col.pszText = (LPWSTR)LoadStr(IDS_KEY_MGMT_COLUMN_BLOCK);    ListView_InsertColumn(hList, 0, &col);
    col.cx = 140; col.pszText = (LPWSTR)LoadStr(IDS_KEY_MGMT_COLUMN_PURPOSE);  ListView_InsertColumn(hList, 1, &col);
