 ...
 * All ESP chips use standard AES-XTS with 128-byte blocks (espsecure implementation).
 *
 ...
 */
int Encrypt_Init(ENCRYPT_CTX *ctx, const BYTE *key, int key_len, DWORD flash_addr)
{
    if (!ctx || !key)
...
int Encrypt_Data(ENCRYPT_CTX *ctx, const BYTE *in_buf, BYTE *out_buf, DWORD len)
{
    ...
    /* Initialize AES with key (AES-128 for 256-bit key, AES-256 for 512-bit key) */
    if (ctx->key_len == 32) {
    ...
    for (offset = 0; offset < len; offset += ENCRYPT_BLOCK_SIZE) {
         ...
         * Reference: espsecure struct.pack("<I", (flash_address & ~0x7F)) + (b"\x00" * 12)
         */
