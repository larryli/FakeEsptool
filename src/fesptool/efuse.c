 ...
 */
static uint32_t read_efuse_bits(const fesp_chip_ctx_t *ctx, int offset, uint32_t mask)
{
 ...
 */
static void write_efuse_bits(fesp_chip_ctx_t *ctx, int offset, uint32_t mask, uint32_t value)
{
    if (!ctx->efuse || offset + 3 >= ctx->efuse_size) {
        return;
    }
...
/*
 * fesp_efuse_apply_block0_defaults - Apply BLOCK0 defaults for missing eFuse fields
 *
...
void fesp_efuse_apply_block0_defaults(fesp_chip_ctx_t *ctx)
{
    ...
    case FESP_CHIP_ESP32:
        ...
        if (ctx->efuse[0x17] == 0) {
            ctx->efuse[0x17] = 0xF0;
            FESP_HAL_LOGD(TAG, "ApplyBlock0Defaults: FLASH_CRYPT_CONFIG set to 0xF");
        }
        break;
    default:
        break;
...
/*
 * fesp_efuse_get_flash_crypt_cnt - Get flash encryption counter value from eFuse
 *
...
/*
 * fesp_efuse_is_download_encrypt_disabled - Check if download mode encryption is
 * disabled
 *
...
bool fesp_efuse_is_download_encrypt_disabled(const fesp_chip_ctx_t *ctx)
{
    ...
    case FESP_CHIP_ESP32S2:
        return read_efuse_bits(ctx, FESP_EFUSE_OFFS_DIS_DL_MANUAL_ENCRYPT_ESP32S2,
                             FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32S2) != 0;
    case FESP_CHIP_ESP32S3:
        return read_efuse_bits(ctx, FESP_EFUSE_OFFS_DIS_DL_MANUAL_ENCRYPT_ESP32S3,
                             FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32S3) != 0;
    case FESP_CHIP_ESP32C2:
        return read_efuse_bits(ctx, FESP_EFUSE_OFFS_DIS_DL_MANUAL_ENCRYPT_ESP32C2,
                             FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C2) != 0;
    case FESP_CHIP_ESP32C3:
        return read_efuse_bits(ctx, FESP_EFUSE_OFFS_DIS_DL_MANUAL_ENCRYPT_ESP32C3,
                             FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C3) != 0;
    case FESP_CHIP_ESP32C6:
        return read_efuse_bits(ctx, FESP_EFUSE_OFFS_DIS_DL_MANUAL_ENCRYPT_ESP32C6,
...
/*
 * fesp_efuse_is_download_decrypt_disabled - Check if download mode decryption is
 * disabled
 *
...
/*
 * fesp_efuse_is_download_mode_disabled - Check if download mode is disabled via eFuse
 *
...
/*
 * fesp_efuse_is_secure_download_enabled - Check if secure download mode is enabled
 *
...
/*
 * fesp_efuse_get_dl_encrypt_disabled - Get raw eFuse value for download encrypt disabled
 */
...
/*
 * fesp_efuse_get_dl_mode_disabled - Get raw eFuse value for download mode disabled
 */
...
uint8_t fesp_efuse_get_key_purpose(const fesp_chip_ctx_t *ctx, int block)
{
    ...
    if (ctx->type == FESP_CHIP_ESP32) {
        if (block == 0)
            return FESP_KEY_PURPOSE_XTS_AES_128_KEY; /* BLOCK1 = flash encryption */
        return FESP_KEY_PURPOSE_USER;
    }
    ...
    int block0_base = (ctx->type == FESP_CHIP_ESP32S2 || ctx->type == FESP_CHIP_ESP32S3 ||
...
/*
 * fesp_efuse_get_encryption_key_offset - Get eFuse offset and length of flash encryption
 * key
 *
...
void fesp_efuse_set_flash_encryption(fesp_chip_ctx_t *ctx, int mode)
{
    ...
    case FESP_CHIP_ESP32:
        ...
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x00, FESP_EFUSE_MASK_FLASH_CRYPT_CNT_ESP32, 1);
        }
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x18, FESP_EFUSE_BIT_DISABLE_DL_ENCRYPT_ESP32, 1);
        }
        break;
    case FESP_CHIP_ESP32S2:
        clear_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32S2);
        clear_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32S2);
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32S2, 1);
        }
        if (mode >= 2)
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32S2,
                           1);
        break;
    case FESP_CHIP_ESP32S3:
        clear_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32S3);
        clear_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32S3);
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32S3, 1);
        }
        if (mode >= 2)
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32S3,
                           1);
        break;
    case FESP_CHIP_ESP32C2:
        clear_efuse_bits(ctx, 0x30, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32C2);
        clear_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C2);
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32C2, 1);
        }
        if (mode >= 2)
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C2,
                           1);
        break;
    case FESP_CHIP_ESP32C3:
        clear_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32C3);
        clear_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C3);
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32C3, 1);
        }
        if (mode >= 2)
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C3,
                           1);
        break;
    case FESP_CHIP_ESP32C6:
        clear_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32C6);
        clear_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C6);
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x34, FESP_EFUSE_MASK_SPI_BOOT_CRYPT_CNT_ESP32C6, 1);
        }
        if (mode >= 2)
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DL_MANUAL_ENCRYPT_ESP32C6,
                           1);
        break;
    default:
        break;
...
void fesp_efuse_set_download_mode(fesp_chip_ctx_t *ctx, int mode)
{
    ...
    case FESP_CHIP_ESP32:
        ...
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x00, FESP_EFUSE_BIT_UART_DOWNLOAD_DIS_ESP32, 1);
        }
        break;
    case FESP_CHIP_ESP32S2:
        ...
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_ENABLE_SECURITY_DL_ESP32S2, 1);
        }
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_DIS_DOWNLOAD_MODE_ESP32S2, 1);
        }
        break;
    case FESP_CHIP_ESP32S3:
        ...
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_ENABLE_SECURITY_DL_ESP32S3, 1);
        }
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_DIS_DOWNLOAD_MODE_ESP32S3, 1);
        }
        break;
    case FESP_CHIP_ESP32C2:
        ...
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_ENABLE_SECURITY_DL_ESP32C2, 1);
        }
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x30, FESP_EFUSE_BIT_DIS_DOWNLOAD_MODE_ESP32C2, 1);
        }
        break;
    case FESP_CHIP_ESP32C3:
        ...
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_ENABLE_SECURITY_DL_ESP32C3, 1);
        }
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_DIS_DOWNLOAD_MODE_ESP32C3, 1);
        }
        break;
    case FESP_CHIP_ESP32C6:
        ...
        if (mode >= 1) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_ENABLE_SECURITY_DL_ESP32C6, 1);
        }
        if (mode >= 2) {
            write_efuse_bits(ctx, 0x3C, FESP_EFUSE_BIT_DIS_DOWNLOAD_MODE_ESP32C6, 1);
        }
        break;
    default:
        break;
