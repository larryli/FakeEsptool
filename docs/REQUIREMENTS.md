...
- **协议层**（`esptool/*.c`）：严格模拟真实设备行为。eFuse 只能 OR 写入（模拟一次性可编程特性），串口信号检测、命令响应、状态机等完全遵循真实 ROM/stub 行为。客户端（esptool/espefuse）通过此层看到的设备行为与真实芯片一致。
