...
| 第4栏 | 加密状态 | 固定 140px |
| 第5栏 | 下载模式 | 固定 120px |
| 第6栏 | 端口（如 `COM10`）或 `Disconnected` | 固定 120px |
| 第7栏 | 串口配置（如 `115200,8N1`） | 固定 140px |
...
| ESP32-C2 | 无 | `ENABLE_SECURITY_DOWNLOAD` | `ENABLE_SECURITY_DOWNLOAD=1` → `Download Secure`，否则 `Download Normal` |
| ESP32-C3 | `DIS_USB_SERIAL_JTAG_DOWNLOAD_MODE` | `ENABLE_SECURITY_DOWNLOAD` | `DIS_USB_SERIAL_JTAG_DOWNLOAD_MODE=1` → `Download Disabled`，`ENABLE_SECURITY_DOWNLOAD=1` → `Download Secure`，否则 `Download Normal` |
| ESP32-C6 | 无 | 无 | 始终 `Download Normal` |
