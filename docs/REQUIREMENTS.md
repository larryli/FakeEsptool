...
| Import eFuse... | 从 QEMU 兼容 .bin 导入 eFuse 块数据 | ESP8266 禁用 |
| Export eFuse... | 导出 eFuse 块数据为 QEMU 兼容 .bin | ESP8266 禁用 |
...
- ESP8266 菜单项禁用（无块结构）
- 弹出文件选择对话框，过滤器为 `*.bin`
- 校验文件大小是否匹配当前芯片的 eFuse 块数据大小
- 不匹配时显示错误提示
- 导入时写入 eFuse 数组对应偏移（仅块读数据区域）
- 导入后设备标记为已修改
...
- 弹出文件保存对话框，过滤器为 `*.bin`
- 导出格式为 QEMU/esp-emulator 兼容的块数据格式
- 各芯片块数据大小：ESP32=124B, C2=84B, S2/S3/C3/C5/C6/C61/H2/P4=336B, S31=316B

### 设备文件格式

- 文件扩展名: `.esp`
- 二进制格式，版本 1（ESP8266）或 版本 2（其他芯片）
...
| 0x04 | 4B | version | 格式版本 (1=ESP8266, 2=其他芯片) |
...
| 0x20 | N | efuse | eFuse 数据（v1:完整数组, v2:块读数据） |
| 0x20+N | M | flash | Flash 数据 |

**v1 vs v2 区别：**

- v1（ESP8266）：eFuse 存储完整寄存器地址空间（288/512 字节），包含 PGM 寄存器和控制寄存器等易失性内容
- v2（其他芯片）：eFuse 仅存储块读数据（ROM 部分），与 QEMU/esp-emulator 格式兼容。加载 v1 文件时自动提取块数据
...
| *(分隔符)* | - | - | - |
| 导入 | toolbar.bmp[7] | Storage/Import Flash（下拉菜单含 Import eFuse） | 始终可用（Import eFuse ESP8266 禁用） |
| 导出 | toolbar.bmp[8] | Storage/Export Flash（下拉菜单含 Export eFuse） | 始终可用（Export eFuse ESP8266 禁用） |
| *(分隔符)* | - | - | - |
| 密钥管理 | toolbar.bmp[9] | Storage/Key Management | ESP8266 禁用 |
| *(分隔符)* | - | - | - |
| 清除 | toolbar.bmp[10] | Log/Clear | 始终可用 |
| 保存日志 | toolbar.bmp[11] | Log/Save as... | 始终可用 |

**工具栏要求：**
- 使用合并的 BMP 位图（12个16x16彩色图标）
- 只显示图标，不显示文本
- 鼠标悬停显示提示文字
