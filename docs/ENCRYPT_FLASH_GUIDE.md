...
量产模式下建议额外烧录的安全 eFuse（防止固件被读取、调试接口被利用）：

| 安全 eFuse | ESP32 | S2 | S3 | C2 | C3/C6 | 说明 |
|-----------|------|-----|-----|-----|-------|------|
| 禁用 JTAG | `JTAG_DISABLE` | `HARD_DIS_JTAG` | `HARD_DIS_JTAG` | `DIS_PAD_JTAG` | `DIS_PAD_JTAG` | 防止通过 JTAG 调试 |
| 禁用 USB JTAG | - | - | `DIS_USB_JTAG` | - | `DIS_USB_JTAG` | 防止 USB 切换到 JTAG |
| 禁用直接启动 | - | `DIS_LEGACY_SPI_BOOT` | `DIS_DIRECT_BOOT` | `DIS_DIRECT_BOOT` | `DIS_DIRECT_BOOT` | 禁用 legacy SPI boot |
| 禁用 UART cache | - | `DIS_DOWNLOAD_ICACHE` | `DIS_DOWNLOAD_ICACHE` | `DIS_DOWNLOAD_ICACHE` | `DIS_DOWNLOAD_ICACHE` | 减少攻击面 |
| 禁用 UART dcache | - | `DIS_DOWNLOAD_DCACHE` | `DIS_DOWNLOAD_DCACHE` | - | - | 减少攻击面 |
| 禁用 UART 下载 | `UART_DOWNLOAD_DIS` | - | - | - | - | ESP32 专用 |
| 安全下载 | - | `ENABLE_SECURITY_DOWNLOAD` | `ENABLE_SECURITY_DOWNLOAD` | `ENABLE_SECURITY_DOWNLOAD` | `ENABLE_SECURITY_DOWNLOAD` | 替代禁用下载 |
| 写保护 | `write-protect-efuse DIS_CACHE` | `write-protect-efuse DIS_ICACHE` | `write-protect-efuse DIS_ICACHE` | - | `write-protect-efuse DIS_ICACHE` | 防止 eFuse 被修改 |

**说明**：
- ESP32：使用 `UART_DOWNLOAD_DIS` 禁用 UART 下载（**最终步骤，烧录后无法再用 espefuse/esptool**）
- S2/S3/C2/C3/C6：使用 `ENABLE_SECURITY_DOWNLOAD` 启用安全下载模式（仍可受限访问）
- 写保护可防止已烧录的安全 eFuse 被意外修改
...
# 5. 测试加密读取（READ_FLASH 返回解密后的明文）
esptool --port COM10 read-flash 0x0 0x1000 flash_read.bin
# 验证：flash_read.bin 内容与 firmware.bin 一致（硬件解密）
...
# 4. 启用量产模式（不可逆，状态栏应显示 "Encrypted (Release)"）
...
# 8. 烧录安全 eFuse（最终步骤，S3 示例，其他芯片见 2.4 节）
espefuse --port COM10 burn-efuse HARD_DIS_JTAG 1
espefuse --port COM10 burn-efuse DIS_DIRECT_BOOT 1
espefuse --port COM10 burn-efuse DIS_USB_JTAG 1
espefuse --port COM10 burn-efuse DIS_DOWNLOAD_ICACHE 1
espefuse --port COM10 burn-efuse ENABLE_SECURITY_DOWNLOAD 1
espefuse --port COM10 write-protect-efuse DIS_ICACHE
```
...
# 8. 烧录安全 eFuse（最终步骤，烧录后无法再用 espefuse/esptool）
espefuse --port COM10 burn-efuse JTAG_DISABLE 1
espefuse --port COM10 burn-efuse UART_DOWNLOAD_DIS 1   # 禁用 UART 下载，烧录后不可逆
espefuse --port COM10 write-protect-efuse DIS_CACHE
```
...
# 8. 烧录安全 eFuse（最终步骤）
espefuse --port COM10 burn-efuse DIS_PAD_JTAG 1
espefuse --port COM10 burn-efuse DIS_DIRECT_BOOT 1
espefuse --port COM10 burn-efuse DIS_DOWNLOAD_ICACHE 1
espefuse --port COM10 burn-efuse ENABLE_SECURITY_DOWNLOAD 1
```
...
READ_FLASH 返回解密后的明文，与真实芯片的硬件解密引擎行为一致。已在开发模式测试（第 3 节）中覆盖。
...
| 禁用下载模式 | `Encrypted (Dev/Release)`, `Download Disabled` |
| 启用安全下载 | `Encrypted (Dev/Release)`, `Download Secure` |
